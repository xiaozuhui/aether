...
## 阶段 2：安全与沙箱能力产品化（DSL 护城河）✅ 已完成

目标：把"可控扩权"做成稳定能力（root_dir 沙箱、内存/DB resolver、缓存生命周期）。

交付：

- ✅ 文件 resolver：root_dir 限制完善 + 越权路径测试（`..`、绝对路径等）- **已完成（2026-01-06）**
- ✅ 模块缓存生命周期：提供显式清理 API；明确与 `reset_env()` 的关系 - **已完成（2026-01-06）**
- ✅ 可观测指标：模块加载次数、缓存命中、TRACE 条数、执行耗时（最小可用）- **已完成（2026-01-06）**

### 已完成功能

**阶段 2.1：路径安全验证（2026-01-06）**
- ✅ 实现 `PathValidator` 统一路径验证
- ✅ 阻止 `..` 路径遍历攻击
- ✅ 阻止绝对路径（可配置）
- ✅ 文件扩展名白名单
- ✅ 所有文件系统函数添加路径验证
...
**阶段 2.2：模块缓存生命周期管理（2026-01-06）**
- ✅ 实现 `ModuleCacheManager`
- ✅ 支持 TTL 和容量限制
- ✅ 提供 `clear()`, `remove()`, `cleanup_expired()` API
- ✅ 提供缓存统计（命中率等）
- ✅ 7 个单元测试

**阶段 2.3：沙箱配置统一（2026-01-06）**
- ✅ 实现 `SandboxConfig` 统一配置入口
- ✅ 提供三种配置预设：`dsl_safe()`, `cli_full_access()`, `sandboxed(root_dir)`
- ✅ 支持完全自定义配置
- ✅ 严格向后兼容

**阶段 2.4：可观测性指标（2026-01-06）**
- ✅ 实现 `MetricsCollector`
- ✅ 执行耗时统计（最小/最大/平均）
- ✅ 模块加载统计（命中/未命中/命中率）
- ✅ 提供 `enable()`, `disable()`, `reset()` API
- ✅ 6 个单元测试

**阶段 2.5：文档与示例（2026-01-06）**
- ✅ 完整的沙箱安全指南（docs/SANDBOX_GUIDE.md）
- ✅ 功能演示示例（examples/sandbox_demo.aether）
- ✅ 集成测试覆盖
